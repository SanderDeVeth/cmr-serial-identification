...
def LoadData(frameObj=None, imgPath=None, maskPath=None):
    imgNames = os.listdir(imgPath)
    maskNames = []

    ## generating mask names
    for mem in imgNames:
        maskNames.append(re.sub('\.jpg', '_seg0.jpg', mem))
    ...
    for i in range(len(imgNames)):
        try:
            ...
            continue
        width, height = 2338, 1700
        dim = (width, height)
        img = cv2.resize(img, dim)
        mask = cv2.resize(mask, dim)
        frameObj['image'].append(img)
...
def Plotter(img, predMask, groundTruth, show_predictions):
...
imgPath = 'Input_left_190/Images'
maskPath = 'Input_left_190/Annotations'
dataFrame = LoadData(dataFrame, imgPath, maskPath)
...
Plotter(actuals[1], sixteenPrediction[1], masks[1], False)
